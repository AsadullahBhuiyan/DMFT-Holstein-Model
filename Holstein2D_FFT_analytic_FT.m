classdef Holstein2D_FFT < handle
    % Contains methods pertinent to solving the 2D Holstein Model
    properties
        kx
        ky
        k_pair_array
    ...
    methods
        ...
        function init_T_dep_objects(self,N_kx,N_ky,N_w,T)
            ...
            self.tau = (0:N_w-1).*self.Beta/N_w;
            self.tau(1) = 1./N_w;
            [~,~,self.tau_zmesh] = meshgrid(self.kx,self.ky,self.tau);
            % tau is the set of discretized imaginary time
            ...
            [~, ~, self.jtau_phase_factor_2] = meshgrid(self.kx,self.ky,exp(1i*pi.*(0:N_w-1))); 
            % imaginary time phase factor
            % Generate phase factors for convenient numerical
            % ordering with FFTs (Sigma_v2)
        ...
        function phase_diagram(self,w_cutoff,N_w_init,n_list,q_max_array,T_SP,T_CDW)
            ...
            for j = 1:length(n_list)
                if ~isnan(T_CDW(j))
                    ...
                    text(n_list(j),(T_CDW(j) + 0.5e-2),string,'FontSize',18);
                end
            end
            grid on
            legend show
            xlabel '$n$'
            ylabel '$T_c$'
            str = [num2str(N_kx),' $\times$ ',num2str(N_ky),' Sites, ','$\omega_{\mathrm{cutoff}}$ = $', num2str(w_cutoff),'t$'];
            str = [str newline '$\lambda_0$ = ',num2str(lambda_0_),', $\omega_E$ = ',num2str(w_E_), ', $U$ = ', num2str(U_)];
            dim = [.15 .025 .3 .3];
            annotation('textbox',dim,'String',str,'FitBoxToText','on',...
                'interpreter','latex','FontSize',16,'linewidth',2,'backgroundcolor','w');            
        ...
        function [T_SP, T_CDW] = linear_ising_fit_plot_Tc(self,w_cutoff,N_w_init,T_list,Chi_SP,Chi_CDW,q_max)
            ...
            dim = [.15 .025 .3 .3];
            str = [num2str(N_kx),' $\times$ ',num2str(N_ky),' Sites, ','$\omega_{\mathrm{cutoff}}$ = $', num2str(w_cutoff),'t$',', $n$ = ',num2str(n_),];
            str = [str newline '$\lambda_0$ = ',num2str(lambda_0_),', $\omega_E$ = ',num2str(w_E_),', $U$ = ',num2str(U_)];
        ...
        function [T_SP, T_CDW] = linear_linear_fit_plot_Tc(self,w_cutoff,N_w_init,T_list,Chi_SP,Chi_CDW,q_max)
            ...
            % Linear Fit to T_SP near Tc
            T_list_near_Tc_SP = T_list(end-1:end).';
            recip_SP_near_Tc_SP = recip_SP(end-1:end);
            fit_SP = fit(T_list_near_Tc_SP,recip_SP_near_Tc_SP,'poly1',...
                'Upper',[inf 0],'Lower',[0 -inf]);
            coeff_vec = coeffvalues(fit_SP);
            T_SP = -coeff_vec(2)/coeff_vec(1);
            ...
            % Ising Fit to T_CDW near Tc
            %idx_T_0p1 = find(abs(T_list - 0.1) == min(abs(T_list - 0.1)));
            T_list_near_Tc_CDW = T_list(end-1:end).';
            recip_CDW_near_Tc_CDW = recip_CDW(end-1:end);
        ...
        function [Chi_out, qx_max, qy_max] = Chi_CDW_bar_maxq(self)
            Pi_vn0_ = self.Pi_vn0;
            Chi_CDW_bar = -1/(self.coupling_constant^2).*real(Pi_vn0_);
        ...
        function out = converge_Lambda(self,F,D_nq)
            ...
            while abs(trial_diff_1) > self.tol_Lambda && divergence_counter < divergence_lim && counter < iter_lim
                ...
                Lambda_calc = 1 - (Lambda_factor).*(ifftn(rx_phase.*ry_phase.*conj(jtau_phase)...
                    .*fftn(D_nq + U_/self.coupling_constant^2).*fftn(F_Lambda_trial)));
        ...
        function [G_out, Sigma_out, D_out, mu_out] = converge_mu(self,mu_1_guess,mu_2_guess)
            ...
            while abs(delta_n_2) > tol_n_ && mu_counter <= iter_lim
                ...
                % Apply Secant method to improve (3+j)rd guess for mu
        ...
        function [G_mk, Sigma_mk, D_nq] = converge_G(self,G0,mu)
            ...
            % CDW_bar_cond = @(x)(max(-((self.lambda_0 - self.U)...
                %/self.coupling_constant^2).*real(x(ceil(end/2),:))));
            ...
            % Analytic Fourier Transforms of G0 and G0_conj
            beta = self.Beta;
            Z_kxkymesh = self.E_kxkymesh - mu;
            tau_mesh = self.tau_zmesh;
            G0_FT = -exp((beta-tau_mesh).*Z_kxkymesh)./(exp(beta.*Z_kxkymesh) + 1);
            G0_FT_inf = -exp(-tau_mesh.*Z_kxkymesh);
            G0_conj_FT = exp(-(beta-tau_mesh).*(Z_kxkymesh))./(exp(-beta.*Z_kxkymesh) + 1);
            if sum(exp(beta.*Z_kxkymesh) == inf,'all')
                G0_FT(exp(beta.*Z_kxkymesh) == inf) = G0_FT_inf(exp(beta.*Z_kxkymesh) == inf);
            end
            ...
            [Sigma_calc, D_kk_mm] = self.Sigma(G_trial,G0,D0_qn,G0_FT,G0_conj_FT);
        ...
        function [Sigma_km, D_qn]= Sigma(self,G_km,G0_km,D0_qn,G0_km_FT,G0_km_conj_FT)
            % FUNCTION DESCRIPTION ----------------------------------------------------
            % Calculates electron self-energy including or excluding phonon-self energy
            % using 3D FFTs to evaluate propagator convolutions
            % Uses analytic FFT of non-interacting Green's function
            % ARGUMENTS ---------------------------------------------------------------
            % G_mk is the single-electron propagator under trial
            % OUTPUTS -----------------------------------------------------------------
            % Sigma_mk is the calculated electron self-energy
            % D_nq is the calculated phonon self-energy propagator
            ...
            factor_Pi = 2*factor_Sigma;
            lambda0 = self.lambda_0;
            
            rx_phase = self.rx_phase_factor;
            ry_phase = self.ry_phase_factor;
            jtau_phase = self.jtau_phase_factor;
            jtau_phase_2 = self.jtau_phase_factor_2;
            
            G0_km_FT_array = cell(N_ky,N_kx);
            nan_matrix = zeros(N_ky,N_kx);
            for j = 1:N_kx
                for jj = 1:N_ky
                    G0_km_FT_array{jj,j} = squeeze(G0_km_FT(j,jj,:));
                    if sum(isnan(G0_km_FT_array{jj,j})) ~= 0
                        nan_matrix(jj,j) = 1;
                    end
                end
            end
            
            % Determine whether to include Pi;
            if isempty(self.renorm_calc)
                Pi_qn = 0;
            else
                %Pi_qn = Pi0_qn + (factor_Pi).*(ifftn(jtau_phase.*rx_phase.*ry_phase.*fftn(G_km).*fftn(conj(G_km)))...
                %- ifft2(jtau_phase.*rx_phase.*ry_phase.*fftn(G0_km).*fftn(conj(G0_km))));
                %Pi_qn = (factor_Pi).*(ifftn(jtau_phase.*rx_phase.*ry_phase.*fftn(G_km).*fftn(conj(G_km))));
                Pi_qn = (factor_Pi).*(ifftn(jtau_phase.*rx_phase.*ry_phase.*...
                    fft(fft(jtau_phase_2.*fft(G_km-G0_km,N_w,3) + beta.*G0_km_FT,N_kx,2),N_ky,1)...
                    .*fft(fft(jtau_phase_2.*fft(conj(G_km-G0_km),N_w,3) + beta.*G0_km_conj_FT,N_kx,2),N_ky,1)));
                if sum(isnan(Pi_qn),'all') ~= 0
                    Pi_qn = (factor_Pi).*(ifftn(jtau_phase.*rx_phase.*ry_phase.*...
                    fft(fft(jtau_phase_2.*fft(G_km,N_w,3),N_kx,2),N_ky,1)...
                    .*fft(fft(jtau_phase_2.*fft(conj(G_km-G0_km),N_w,3),N_kx,2),N_ky,1)));
                end
            end
            
            %
            chi_bar = -1/self.coupling_constant^2*real(Pi_qn(:,:,ceil(end/2)));
            
            if ~isempty(chi_bar*lambda0 >= 1) && self.D_divergence_counter <= 15
                chi_bar(chi_bar*lambda0 >= 1) = 1-1e-8;
                Pi_qn(:,:,ceil(end/2)) = -self.coupling_constant^2*chi_bar;
            ...
            %Sigma_km = Sigma0_km + -(factor_Sigma).*(ifftn(conj(jtau_phase).*rx_phase.*ry_phase.*...
                %fftn(D_qn).*fftn(G_km)) - ifftn(conj(jtau_phase).*rx_phase.*ry_phase.*fftn(D0_qn).*fftn(G0_km)));
            Sigma_km = -(factor_Sigma).*ifftn(conj(jtau_phase).*rx_phase.*ry_phase.*fftn(D_qn)...
                .*conj(jtau_phase_2).*fft(fft(jtau_phase_2.*fft(G_km-G0_km,N_w,3) + beta.*G0_km_FT,N_kx,2),N_ky,1));
            if sum(isnan(Sigma_km),'all') ~= 0
                Sigma_km = -(factor_Sigma).*ifftn(conj(jtau_phase).*rx_phase.*ry_phase.*fftn(D_qn)...
                .*conj(jtau_phase_2).*fft(fft(jtau_phase_2.*fft(G_km,N_w,3),N_kx,2),N_ky,1));
            end
        end
        ...
        function out = Sigma0(self,mu)
            ...
            out = (self.coupling_constant^2/N_k).*repmat(Sigma0_m,[N_ky N_kx 1]);
        end
        ...
        function out = Pi(self,G_km)
            ...
            % Generate phase factors for convenient numerical
            % ordering with FFTs
            [rx_phase, ry_phase, jtau_phase] = ...
                meshgrid(exp(-1i*pi*(2/N_kx - 1).*(0:N_kx-1)),... % x real space phase factor
                exp(-1i*pi*(2/N_ky - 1).*(0:N_ky-1)),... % y real space phase factor
                exp(-1i*pi*(N_w-1)/N_w.*(0:N_w-1))); % imaginary time phase factor
            
            out = (factor_Pi).*(ifftn(jtau_phase.*rx_phase.*ry_phase.*fftn(G_km).*fftn(conj(G_km))));
            
            
        end
        
        function out = Sigma_G(self,G_km,G0_km,Sigma_km)
            N_kx = length(self.kx);
            N_ky = length(self.ky);
            N_k = N_kx*N_ky;
            N_w = length(self.w_m);
            ...
            out = Sigma0_km + -(factor_Sigma).*(ifftn(conj(jtau_phase).*rx_phase.*ry_phase.*...
                fftn(D_qn).*fftn(G_km)) - ifftn(conj(jtau_phase).*rx_phase.*ry_phase.*fftn(D0_qn).*fftn(G0_km)));
