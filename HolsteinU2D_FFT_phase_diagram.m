...
H.tol_Lambda = 1e-8; % tolerance for self-consistent vertex calculation of Lambda
...
U_list = [0 2.5 4];
w_E_list = [1e-3 1e-2];
for jjjj = 1:length(U_list)
    H.U = U_list(jjjj); % Hubbard U term
    for jjj = 1:length(w_E_list)
        H.w_E = w_E_list(jjj); % phonon frequency
        H.gwE = sqrt(H.lambda_0*H.w_E/2); %gw_E
        n_list = [0.01 0.1:0.1:1.0];
        ...
        for jj = 1:length(n_list)
            T_list = linspace(0.001,0.25,251);
            T_list = flip(T_list);
            ...
            if ~isempty(q_x) && ~isempty(q_y(end))
